...
<script lang="ts">
    ...
    let {
        ...
        data: { recipe: Recipe; batches: Batch[]; queryBatchId: string | null };
    } = $props();
    ...
    const selected = $derived(
        data.batches.find((b) => b.id === selectedId) ?? null,
    );
...
{#if data.recipe}
    <div class="max-w-7xl mx-auto p-4 md:p-6 flex flex-col gap-4 min-h-screen">
        <nav class="flex items-center gap-2 text-sm">
            <a
                href={resolve("/")}
                class="inline-flex items-center gap-1 text-obsidian/60 hover:text-obsidian"
            >
                <svg
        ...
        <header
            class="flex md:items-end justify-between border-b border-drafting pb-3 flex-col md:flex-row gap-3"
        >
            <div>
                <h1 class="font-serif text-3xl">{data.recipe.name}</h1>
                {#if data.recipe.description}
                    <p class="text-sm text-obsidian/60 mt-1">
                        {data.recipe.description}
                    </p>
                {/if}
            </div>
            <div class="flex md:items-end gap-3 flex-wrap md:flex-nowrap">
                {#if data.recipe.tags.length}
                    <div
                        class="flex gap-1.5 text-[10px] uppercase tracking-wider text-obsidian/60 w-full md:w-auto"
                    >
                        {#each data.recipe.tags as t (t)}<span
                                class="border border-drafting px-2 py-0.5 rounded-sm"
                                >{t}</span
                            >{/each}
                    </div>
                {/if}
                <Button
                    variant="ghost"
                    size="sm"
                    class="border border-drafting hover:border-ochre"
                    onclick={() => (editVarsOpen = true)}
                    data-testid="edit-variables-btn">Edit Variables</Button
                >
                <Button
                    variant="outline"
                    size="sm"
                    onclick={() => (deleteDialogOpen = true)}
                    data-testid="delete-recipe-btn">Delete Recipe</Button
                >
            </div>
        </header>
        ...
        {#if data.batches.length === 0}
            ...
            >
                <p class="text-sm text-obsidian/60">
                    No batches yet. Record your first one to get started.
                </p>
                <a
                    href={resolve(`/recipes/${data.recipe.id}/new-batch`)}
                    class="border border-ochre text-ochre px-4 py-2 text-sm uppercase tracking-wider hover:bg-ochre hover:text-canvas rounded-sm"
                    >+ Record V1</a
                >
            </div>
        {:else}
            <div
                class="flex-1 flex flex-col lg:grid lg:grid-cols-[340px_1fr] lg:gap-6 min-h-0"
            >
                <!-- Mobile-only tab bar -->
                <div class="border-b border-drafting flex lg:hidden">
                    <button
                        ...
                        class="flex-1 px-4 py-3 text-xs uppercase tracking-wider text-center {mobileTab ===
                        'batches'
                    ...
                    <button
                        ...
                        class="flex-1 px-4 py-3 text-xs uppercase tracking-wider text-center truncate {mobileTab ===
                        'detail'
                            ? 'text-ochre border-b-2 border-ochre -mb-px font-bold'
                            : 'text-obsidian/60 hover:text-obsidian'}"
                ...
                <aside
                    class="overflow-auto flex flex-col items-center lg:items-stretch lg:border-r lg:border-drafting lg:pr-6 {mobileTab ===
                    'batches'
                        ? ''
                        : 'hidden'} lg:block"
                >
                ...
                <section
                    class="{mobileTab === 'detail' ? '' : 'hidden'} lg:block"
                >
                    {#if selected}
                        <BatchDetail
                            recipe={data.recipe}
    ...
    {#if editingOutcome}
        <OutcomeForm
            batch={editingOutcome}
            recipeId={data.recipe.id}
            mode="edit"
            onClose={() => (editingOutcome = null)}
        />
    {/if}
    ...
    <ConfirmDeleteDialog
        ...
        body="This permanently deletes the recipe and all {data.batches
            .length} batch{data.batches.length === 1
            ? ''
            : 'es'}. This can't be undone. Type the recipe name to confirm."
